...
void UartReadFrame(UART_TYPE *uart)
{
    ...
    uint16_t consumed_len,payload_len;
    #if uartTA_PROTOCOL_ENABLED
    ...
    if(uart == &Uart2)
    {
        rx_buffer = Uart2RxBuffer;
        rx_buffer_size = uartUART2_RXBUF_SIZE;
    }
    #endif /* uartUART2_ENABLED */
    #if uartUART3_ENABLED
    if(uart == &Uart3)
    {
        rx_buffer = Uart3RxBuffer;
        rx_buffer_size = uartUART3_RXBUF_SIZE;
    }
    #endif /* uartUART3_ENABLED */
    #if uartUART4_ENABLED
    if(uart == &Uart4)
    {
        rx_buffer = Uart4RxBuffer;
        rx_buffer_size = uartUART4_RXBUF_SIZE;
    }
    #endif /* uartUART4_ENABLED */
    #if uartUART5_ENABLED
    if(uart == &Uart5)
    {
        rx_buffer = Uart5RxBuffer;
        rx_buffer_size = uartUART5_RXBUF_SIZE;
    }
    #endif /* uartUART5_ENABLED */

    if((rx_buffer == NULL) || (rx_buffer_size == 0U))
    {
        return;
    }
    ...
    if(rx_head_bak == rx_tail_bak)
    {
        uart->RxFlag = UART_NON_REC;
        return;
    }

    uart->RxFlag = UART_RECING;
    i = 0U;
    while((rx_head_bak != rx_tail_bak) && (i < uartUART_COMMON_FRAME_SIZE))
    {
        frame[i++] = rx_buffer[rx_tail_bak++];
        if(rx_tail_bak >= rx_buffer_size)
        {
            rx_tail_bak = 0U;
        }
    }
    ...
    while(i > 0U)
    {
        ...
        if(frame[frame_offset] == 0x5a && frame[frame_offset + 1] == 0xa5)
        {
            ...
            one_frame_len = (uint16_t)frame[frame_offset + 2] + 3U;
            if(i < one_frame_len)
            {
                if(uart->RxTimeout != 0U)
                {
        ...
        }else if(frame[frame_offset] == 0xaa && frame[frame_offset + 1] == 0x55)
        {
            ...
            payload_len = ((uint16_t)frame[frame_offset + 2] << 8) |
                          frame[frame_offset + 3];
            if(payload_len > (uint16_t)(uartUART_COMMON_FRAME_SIZE - 4U))
            {
                i--;
                continue;
            }
            one_frame_len = payload_len + 4U;
            if(i < one_frame_len)
            {
                if(uart->RxTimeout != 0U)
                {
        ...
        else if(frame[frame_offset] == modbusSLAVE_ADDRESS)
        {
            ...
            one_frame_len = (uint16_t)frame[frame_offset + 2] + 5U;
            if(i < one_frame_len)
            {
                if(uart->RxTimeout != 0U)
                {
        ...
        else if(frame[frame_offset] == 0xAA)
        {
            ...
            ta_raw_len = ((uint16_t)frame[frame_offset + 1] << 8) |
    ...
    consumed_len = total_frame_len - i;
    if(consumed_len > 0U)
    {
        rx_tail_bak = uart->RxTail + consumed_len;
        if(rx_tail_bak >= rx_buffer_size)
        {
            rx_tail_bak -= rx_buffer_size;
        }
